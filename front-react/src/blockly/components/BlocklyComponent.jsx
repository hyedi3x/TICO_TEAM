...
function Canvas() {
  ...
  const textArr = useRef([]); // 글상자 리스트 추가
  ...
  function textDel(index) {
    textArr.current.splice(index, 1); // 글상자 배열에서 삭제
    callImgArr(); // 캔버스 다시 그리기
  }
  ...
  const callimage = (imgUrl) => {
    ...
    if(imgUrl === 'https://tico.kro.kr/uploads/entrybot.png'){
      img.src = imgUrl;
    } else {
  ...
  const callImgArr = () => {
    ...
    // 모든 오브젝트 (이미지, 복제된 이미지, 글상자)
    const allObjects = [...imgArr.current, ...(window.cloneArr || []), ...textArr.current];
    ...
    allObjects.forEach((item) => {
      ...
      if (item.img) {
        ...
        // 글상자 그리기
        context.font = `${item.fontSize || 24}px ${item.fontFamily || 'Arial'}`;
  ...
  const handleMouseDown = (e) => {
    ...
    // 텍스트 클릭 체크
    if (foundIndex === -1) {
      textArr.current.forEach((refItem, index) => {
        if (
          offsetX >= refItem.x &&
          offsetX <= refItem.x + refItem.width &&
          offsetY >= refItem.y &&
          ...
          foundIndex = `text-${index}`;   // 텍스트는 'text-번호' 로 구분
          isText = true;
          startPosRef.current = { x: offsetX - refItem.x, y: offsetY - refItem.y };
        }
      });
  ...
  const handleMouseMove = (e) => {
    ...
    if (selectedImageIndex !== null &&
      ...
      // 🔥 selectedImageIndex 가 "text-" 인지 아닌지에 따라
      if (typeof selectedImageIndex === 'string' && selectedImageIndex.startsWith('text-')) {
        const textIdx = parseInt(selectedImageIndex.split('-')[1]);
        // 선택된 인덱스의 좌표를 업데이트
        textArr.current[textIdx].x = offsetX - startPosRef.current.x;
        textArr.current[textIdx].y = offsetY - startPosRef.current.y;
      } else {
      ...
      <div className="editor-wrapper">
        ...
        <div className='canvas-and-objects'>
          ...
          <div className="object-panel-container" style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', marginTop: '1rem', position: 'relative' }}>
            ...
            {textArr.current.map((obj, index) => (
              <div key={`text-${index}`} className="object-panel-item">
                <ObjectControlPanel
                  object={obj}
                  onUpdate={(updatedObj) => {
                    textArr.current[index] = updatedObj;
                    callImgArr();
                  }}
                  i={index}
                  onDelete={() => textDel(index)}
                  isSelected={selectedImageIndex === `text-${index}`}
                  onClick={() => setSelectedImageIndex(`text-${index}`)}
                />
              </div>
            ))}
            {/* 오버레이 */}
          ...
          <ObjectSelectPage
            onComplete={(selectedObjects) => {
              ...
              selectedObjects.forEach(obj => {
                if (obj.type === 'text') {
                  textArr.current.push({
                    id: Date.now() + Math.random(),   // 고유 ID
                    text: obj.text,
                    fontSize: obj.fontSize,
                    color: obj.color,
                    fontFamily: obj.fontFamily,
                    x: canvasRef.current.width / 2 - 50,
                    y: canvasRef.current.height / 2 - 25,
                    width: 100,
                    height: 50,
                    angle: 0,
                    hidden: false,
                    index: textArr.current.length,  // index 추가
                    isText: true                    // 텍스트 구분용 플래그 추가 (선택)
                  });
                  callImgArr();
                } else {
                  callimage(obj.blocklyObjectFilePath); // 기존 이미지 오브젝트
                }
              });
            }}
          />
