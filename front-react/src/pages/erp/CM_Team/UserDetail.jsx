...
const UserDetail = ({ uuid, onBack, onEdit }) => {
  ...
  // 에러 메시지 출력
  if (errorMsg) {
    return <Message type="error" showIcon>{errorMsg}</Message>;
  }
  ...
  return (
    <div className="user-detail-wrapper">
      {/* 사용자 기본 정보 */}
      <Panel bordered header={`${user.name} 님의 상세 정보`} className="user-detail-panel">
        <table className="user-info-table">
          <tbody>
            ...
            <tr><th>가입 방식</th><td>{user.provider}</td></tr>
          </tbody>
        </table>
        ...
        <div className="back-button">
          <Button onClick={onBack} appearance="subtle">← 목록으로 돌아가기</Button>
          <Button onClick={() => onEdit(user.userUuid)} appearance="primary" style={{ marginLeft: 10 }}>
            ✏️ 회원 정보 수정
          </Button>
        </div>
      </Panel>
